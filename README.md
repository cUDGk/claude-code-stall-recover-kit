...
今回の `3bac9d8e-9ec4-4cdf-98c7-1be6d5ac4069` のように、巨大化した transcript に壊れたXMLツール呼び出しが何度も残っている場合、通常作業をそのまま続けるのは危険。

復旧順序は次の通り：

1. idle 状態で `/compact` を試す。漏れた `<invoke>` 実例が要約で消えるため、効くケースがある。
2. まだ `court` / `<invoke>` が出る場合は `/model` で Sonnet 系へ切り替える。
3. それでもダメなら `/clear` または新規セッションへ逃がす。

`session_health_guard.py` は通常作業プロンプトを止めるが、復旧用の `/compact` / `/clear` / `/rewind` / `/model` は通す。
...
- 一度XML漏れが複数回出たセッションは、まず `/compact`、ダメならモデル切替か新規セッションへ逃がす
- Bash コマンドは短く保つ
- `;` や `&&` でのコマンド連結を避ける
